...
class RandomMNISTDataloader(pl.LightningDataModule):
    def __init__(self, batch_size, path, p=0.0, transform=None):
        ...
        self.train_ds = RandomMNISTDataset(path, p=self.p, transform=transform)

        self.val_ds = MNIST(path, train=False, download=True, transform=transform)
